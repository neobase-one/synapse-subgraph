...
export function getReceivedValue(
  amount: BigInt,
  token: Address,
...
export function getSwapPoolCoinAddresses(
  poolAddress: Address,
  index: i32
): Address {
...
export function getReceivedToken(
  kappa: string,
  pool: Address,
  swapSuccess: bool,
  tokenIndexTo: i32,
...
export function getPoolAddress(type: string, input: Bytes): Address {
  ...
  let withdrawAndRemoveTypestring =
    '(address,address,uint256,uint256,address,uint8,uint256,uint256,bytes32)'
  // mintAndSwap
  // to, token, amount, fee, pool, tokenIndexFrom, tokenIndexTo, minDy, deadline, kappa
  let mintAndSwapTypestring =
    '(address,address,uint256,uint256,address,uint8,uint8,uint256,uint256,bytes32)'
